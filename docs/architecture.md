...
- Calls the backend exclusively through the `core/services/` layer
  (`EmployeeService`, `DepartmentService`, `RoleService`, `DashboardService`),
  which use Angular's `HttpClient` and share a single functional
  `errorInterceptor` for normalized error handling.
...
EmployeeFormComponent (submit)
  ...
  -> Angular router navigates back to the employee list
```
...
DashboardComponent (ngOnInit)
  ...
  -> Angular renders stat cards, the department bar chart and recent list
```

## Why no Spring Security / JPA / microservices

This project is intentionally scoped to the developer's current, verifiable
skill set (Java, JDBC, Spring Boot REST, Angular) so that every layer can be
